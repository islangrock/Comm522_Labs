...
*This lab reviews the construction and analysis of scales as well as two kinds of reliability (internal reliability and test-retest reliability)* 

## Objectives 

This week's lab focuses on an important feature of research (particularly survey research): the construction and analysis of scales made with a group of similar items. The lab will also introduce data from the American National Election Survey, a popular and important source. 
...
We will use tidyverse (hopefully you are starting to feel familiar with tidyverse by now!), psych, and car.
...
This lab uses a dataset compiled from a set of questions on the 2012 American National Election Survey. Each of these questions can be used to construct different scales, and then we can measure their reliability and validity. 

Annenberg students have access to the full survey and survey data if you make an account at the ANES data center website. Additional information about the survey can be found on the website as well. 
...
```{r}
anes<- read.csv("~/Desktop/Methods_Rlabs/data/anes_2012.csv") # Remember to change path as needed
```

## Additional Resources 
  - as always, check out R For Data Science! 
  - Helpful "Cheatsheets"
...
Our first set of questions are about the environment. In this section we'll construct a scale with these questions then test the internal reliability. 

## Variables of Interest 

The environment scale will use 5 questions about various environment realted questions. The names and descriptions (paraphrased from the ANES codebook): 
...
To make our analysis easier, let's create a new data frame that includes only these environment variables. 
```{r}
...
We will use the alpha() function from the psych package 
...
# Example 2: Equalitarianism Scale 
...
First, like above, let's create a new data set with just these variables 
...
Since all the questions are on the same scale we don't need to do the big recoding that we did forthe environmental scale. But we do need to account for the "reverse worded" questions marked above. These are the instances where strongly agreeing is less egalitarian than the other questions. We can do this in the keys command of the alpha function. 
...
This type of validity asks if the measure predicts what it is supposed to? 
...
Like in the examples above, we'll need to do some recoding before we can look at the correlations. 
